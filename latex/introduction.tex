...
since the solution methods rely on randomized algorithms~\cite{Shmoys2004}. Conservativeness
is an advantage in conceptual design, since we would like to have some guarantees
that a design can handle uncertainty in parameters.

\subsubsection{Stochasticity}

The solution of \gls{ro} problems is deterministic,
meaning that different instances of a design problem with
the same parameters will result in the same solution.
This is not the case with \gls{so},
since the optimum depends on realizations of random variables. This
is not satisfactory from an engineering perspective, since a design
can be sensitive to issues in sampling schemes. Furthermore,
optimization runs over the same parameters may result in different
solutions. In the context of engineering design determinism makes
\gls{ro} superior with respect to \gls{so}.
